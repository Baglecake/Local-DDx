...
class DiagnosisSynthesizer:
    ...
    def _apply_credibility_selection(self, master_list: List[str],
        ...
        # --- NEW: NORMALIZE AND CONSOLIDATE SCORES FIRST ---
        normalized_scores = {}
        normalized_rankings = {}

        for diagnosis, score in borda_scores.items():
            # Use the existing normalization function
            normalized_name = self._normalize_diagnosis(diagnosis)
            # Add the score to the normalized entry
            normalized_scores[normalized_name] = normalized_scores.get(normalized_name, 0) + score

        # Also normalize the rankings for consistency
        for agent_name, ranking in all_rankings.items():
            normalized_ranking = []
            for diagnosis in ranking:
                normalized_name = self._normalize_diagnosis(diagnosis)
                if normalized_name not in normalized_ranking:  # Avoid duplicates within same agent
                    normalized_ranking.append(normalized_name)
            normalized_rankings[agent_name] = normalized_ranking

        print(f"   ✅ Normalized {len(borda_scores)} raw votes into {len(normalized_scores)} unique diagnoses.")
        ...
        for diagnosis, base_score in normalized_scores.items():  # Use normalized_scores instead of borda_scores
            weighted_score = 0
            supporting_agents = []

            # Find which agents ranked this diagnosis and their positions
            for agent_name, ranking in normalized_rankings.items():  # Use normalized_rankings instead of all_rankings
