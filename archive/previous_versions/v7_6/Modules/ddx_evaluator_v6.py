...
class ClinicalEquivalenceAgent:
    ...
    {{"equivalent": true/false, "confidence": 0.8, "reasoning": "Brief clinical justification citing any transcript evidence"}}
    ...
    def _parse_equivalence_response(self, content: str, sys_diag: str, gt_diag: str,
        ...
        """Parse the agent's equivalence evaluation response - handles both JSON and text"""
        is_equivalent = False
        confidence = 0.0
        reasoning = "No reasoning provided"

        try:
            # Method 1: Try JSON parsing first (for Qwen2 and other JSON-outputting models)
            import json
            import re
            ...
            if json_match:
                ...
                if isinstance(data, dict):
                    ...
                    reasoning = data.get('reasoning', 'JSON response provided')
                    
                    print(f"      ✅ JSON equivalence parsed: equivalent={is_equivalent}, confidence={confidence:.2f}")
                    return EquivalenceResult(
                        is_equivalent=is_equivalent,
            ...
            # Method 2: Fallback to original text parsing
