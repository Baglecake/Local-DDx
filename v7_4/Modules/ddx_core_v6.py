...
class DDxAgent:
            ...
            for json_obj in json_objects:
                if isinstance(json_obj, dict) and len(json_obj) > 0:
                    ...
                    if self._is_valid_medical_json(json_obj):
                        ...
                        print(f"   ✅ {self.name}: Successfully extracted {len(json_obj)} diagnoses from JSON")
                        return json_obj
    ...
    def _find_json_objects(self, content: str) -> List[Dict]:
        """Find all potential JSON objects in content, handling Llama3 chattiness"""
        import re
        import json
        
        json_objects = []
        
        # Pattern 1: Find complete JSON objects with proper nesting
        json_pattern = r'\{[^{}]*(?:\{[^{}]*\}[^{}]*)*\}'
        ...
        # Pattern 2: Look for JSON-like structures that span lines
        lines = content.split('\n')
        for i, line in enumerate(lines):
            if '{' in line and ':' in line:
                # Try to build JSON from this point
                json_candidate = ""
                brace_count = 0
                
                for j in range(i, min(i + 20, len(lines))):  # Max 20 lines
                    json_candidate += lines[j] + " "
                    brace_count += lines[j].count('{') - lines[j].count('}')
                    
                    if brace_count == 0 and '{' in json_candidate:
                        try:
