...
class RefinementAndJustificationRound(BaseRound):
    ...
    def execute(self, agents: List[DDxAgent], case_description: str,
           context: Optional[Dict[str, Any]] = None,
           global_transcript: Optional[Dict] = None) -> RoundResult:
        """Execute enhanced collaborative debate with sliding context"""

        print(f"\n💬 ENHANCED REFINEMENT AND JUSTIFICATION ROUND")
        print("=" * 50)
        ...
        # Enhanced collaborative debate with sliding context
        responses = {}
        all_refinements = []
        debate_interactions = []
        ...
        try:
            ...
            # SEQUENTIAL DEBATE - agents respond to each other
            for i, agent in enumerate(agents):
                # Build sliding context for this agent
                context_string = ""
                if global_transcript and i > 0:  # After first agent
                    try:
                        from ddx_sliding_context import SlidingContextManager, ContextFilterType
                        context_manager = SlidingContextManager()
                        context_string = context_manager.build_context_for_agent(
                            agent.name, agent.specialty.value, "refinement_and_justification",
                            global_transcript,
                            [ContextFilterType.OPPOSING_VIEWS, ContextFilterType.HIGH_CONFIDENCE, ContextFilterType.KEY_EXCHANGES]
                        )
                    except Exception as e:
                        print(f"⚠️ Context building failed for {agent.name}: {e}")

                # Systematic refinement prompt
                systematic_prompt = f"""
                You are participating in a systematic medical case review. Address EVERY diagnosis on the master list systematically.
                ...
                PART 1 - Systematic Review (address EVERY diagnosis):

                <SYSTEMATIC_REVIEW>
                {chr(10).join([f"{i+1}. {diag}: [SUPPORT/CHALLENGE/NEUTRAL/DEFER] - [Brief clinical reasoning]" for i, diag in enumerate(master_diagnoses)])}
                </SYSTEMATIC_REVIEW>

                PART 2 - Detailed Analysis (your top 3 most supported):

                <DETAILED_ANALYSIS>
                1. **[Diagnosis]** - [Confidence: HIGH/MEDIUM/LOW] - [Full evidence-based reasoning]
                2. **[Diagnosis]** - [Confidence: HIGH/MEDIUM/LOW] - [Full evidence-based reasoning]  
                3. **[Diagnosis]** - [Confidence: HIGH/MEDIUM/LOW] - [Full evidence-based reasoning]
                </DETAILED_ANALYSIS>

                As a {agent.specialty.value} specialist, systematically address every diagnosis, then provide detailed analysis of your most supported candidates.
                """

                response = agent.generate_response(systematic_prompt, "refinement_and_justification", global_transcript)
                responses[agent.name] = response

                # Extract enhanced refinements
                refinements = self._extract_enhanced_refinements(response.content, agent.name)
                all_refinements.extend(refinements)

                # Detect debate interactions
                interactions = self._detect_debate_interactions(response.content, agent.name, i)
                debate_interactions.extend(interactions)

                print(f"✅ {agent.name} completed evidence-based analysis")

                # Add this response to transcript for next agent
                if global_transcript:
                    ...
                    if 'refinement_and_justification' not in global_transcript['rounds']:
                        global_transcript['rounds']['refinement_and_justification'] = {
                            'responses': {},
                            'round_type': 'refinement_and_justification'
                        }

                    global_transcript['rounds']['refinement_and_justification']['responses'][agent.name] = {
            ...
            # Enhanced analysis of debate quality
            refinement_data = self._analyze_enhanced_debate(all_refinements, responses, debate_interactions)

            print(f"\n📊 Enhanced Debate Summary:")
            print(f"   💬 Evidence-based interactions: {refinement_data.get('evidence_interactions', 0)}")
            print(f"   🎯 Challenges raised: {refinement_data.get('challenges_detected', 0)}")
            print(f"   🏥 Specialty insights: {refinement_data.get('specialty_insights', 0)}")
            print(f"   📈 Debate quality score: {refinement_data.get('debate_quality_score', 0):.2f}")
    ...
    def _extract_enhanced_refinements(self, content: str, agent_name: str) -> List[Dict[str, Any]]:
        """Extract both systematic reviews and detailed analysis"""
        refinements = []

        # Extract systematic reviews - these cover ALL diagnoses
                ...
                if re.match(r'^\d+\.', line):
                    ...
                    if len(parts) >= 2:
                        ...
                        refinements.append({
                            ...
                            'stance': stance,
                            'reasoning': reasoning,
                            'confidence': 'MEDIUM',  # Default for systematic reviews
                            'type': 'systematic'
                        })

        # Extract detailed analysis - these are the top choices
        detailed_match = re.search(r'<DETAILED_ANALYSIS>(.*?)</DETAILED_ANALYSIS>', content, re.DOTALL)
        if detailed_match:
            ...
            for line in lines:
                ...
                if re.match(r'^\d+\.', line):
                    ...
                    try:
                        # Extract diagnosis name (between ** markers or first part)
                        if '**' in line:
                        ...
                        # Extract reasoning (everything after confidence)
                        reasoning_match = re.search(r'Confidence:\s*(?:HIGH|MEDIUM|LOW)\s*-\s*(.*)', line, re.IGNORECASE)
                        ...
                        if diagnosis:
                            refinements.append({
                                ...
                                'diagnosis': diagnosis,
                                'recommendation': f"{diagnosis} - {reasoning}",
                                'stance': 'SUPPORT',  # Detailed analysis implies support
                                'reasoning': reasoning,
                                'confidence': confidence,
                                'type': 'detailed'
                            })
                        ...
                        print(f"⚠️ Failed to parse detailed refinement line: {line[:50]}...")
                        continue

        print(f"   📋 {agent_name}: Extracted {len(refinements)} total refinements")
        return refinements
