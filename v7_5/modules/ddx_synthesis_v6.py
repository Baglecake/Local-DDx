...
class DiagnosisSynthesizer:
    ...
    def _apply_credibility_selection(self, master_list: List[str],
        ...
        # Normalize and consolidate diagnosis names before scoring
        normalized_borda_scores = {}
        normalized_rankings = {}
        
        # Normalize Borda scores by consolidating duplicate diagnoses
        for diagnosis, score in borda_scores.items():
            normalized_name = self._normalize_diagnosis_for_synthesis(diagnosis)
            if normalized_name in normalized_borda_scores:
                normalized_borda_scores[normalized_name] += score
            else:
                normalized_borda_scores[normalized_name] = score
        
        # Normalize rankings by updating diagnosis names
        for agent_name, ranking in all_rankings.items():
            normalized_ranking = []
            for diagnosis in ranking:
                normalized_name = self._normalize_diagnosis_for_synthesis(diagnosis)
                if normalized_name not in normalized_ranking:  # Avoid duplicates in ranking
                    normalized_ranking.append(normalized_name)
            normalized_rankings[agent_name] = normalized_ranking

        provisional_list = []

        # Apply credibility weighting to normalized Borda scores
        print(f"\n📊 Applying credibility-weighted scoring:")

        credibility_weighted_scores = {}

        for diagnosis, base_score in normalized_borda_scores.items():
            weighted_score = 0
            supporting_agents = []

            # Find which agents ranked this diagnosis and their positions
            for agent_name, ranking in normalized_rankings.items():
    ...
    def _normalize_diagnosis_for_synthesis(self, diagnosis: str) -> str:
        """Normalize diagnosis name for synthesis - removes prefixes and standardizes format"""
        import re
    ...
    def _consolidate_similar_diagnoses(self, provisional_list: List[str]) -> List[str]:
        ...
        for diagnosis in provisional_list:
            ...
            # Normalize this diagnosis for comparison
            normalized_current = self._normalize_diagnosis_for_synthesis(diagnosis)
            
            # Check for similar diagnoses to consolidate
            similar_found = False
            current_lower = normalized_current.lower()

            for other in provisional_list:
                if other != diagnosis and other not in processed:
                    normalized_other = self._normalize_diagnosis_for_synthesis(other)
                    other_lower = normalized_other.lower()

                    # Check for exact match after normalization
                    if normalized_current == normalized_other:
                        # Keep the cleaner version (shorter usually means less prefixed)
                        ...
                        processed.add(other)
                        similar_found = True
                        print(f"   🔄 Consolidated: '{diagnosis}' + '{other}' → '{final_list[-1]}'")
                        break
                    
                    # Check semantic similarity
                    elif self._should_consolidate(current_lower, other_lower):
                        ...
                        processed.add(other)
                        similar_found = True
                        print(f"   🔄 Consolidated similar: '{diagnosis}' + '{other}' → '{final_list[-1]}'")
                        break
